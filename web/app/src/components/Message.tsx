...
function MessageRoot({ role, name, avatar, agentColor = 'primary', isStreaming, timestamp, metadata, children }: MessageProps) {
  if (role === 'system') {
    return (
      <div className="flex justify-center my-6">
        <div className="bg-brand-bg rounded-lg px-4 py-2 border border-brand-border">
          <div className="text-sm text-[#859289] font-medium">{children}</div>
        </div>
      </div>
    );
  }
  ...
  if (role === 'user') {
    return (
      <div className="flex gap-3 mb-6 animate-fadeIn">
        <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-lg bg-gradient-to-br from-brand-accent to-[#d06c6e] flex-shrink-0">
  ...
  return (
    <div className={`flex gap-3 mb-6 animate-fadeIn ${isStreaming ? 'opacity-90' : ''}`}>
      <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${colors.avatar} flex-shrink-0`}>
        <span className="text-xl">{avatar || '🤖'}</span>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2 mb-2">
          <span className={`font-semibold text-base ${colors.name}`}>
            {name || 'Agent'}
          </span>
          {metadata && (
            <span className="text-xs text-[#859289]">{metadata}</span>
          )}
          {timestamp && (
            <span className="text-xs text-[#859289]">
        ...
        <div className={`bg-brand-card bg-opacity-40 rounded-xl p-4 border ${colors.border} border-opacity-20 hover:shadow-xl transition-shadow`}>
          <div className="text-[#d3c6aa] leading-relaxed text-base prose prose-invert prose-p:leading-relaxed prose-pre:bg-[#2b3339] prose-pre:border prose-pre:border-brand-border max-w-none">
            ...
            {isStreaming && <span className="inline-block w-2 h-5 bg-[#859289] ml-1 animate-pulse" />}
          </div>
        </div>
      </div>
    </div>
